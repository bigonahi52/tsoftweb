...
function StatBox({ v, suffix, plain, label }: { v: number; suffix?: string; plain?: boolean; label: string }) {
  const { ref, text } = useCountUp(v, 1400, plain);
  return (
...
const VALUES = [
  ...
  { icon: "shield", title: "داده مشتری، امانت است", desc: "هیچ داده‌ای بدون اجازه مشتری جابه‌جا نمی‌شود. پشتیبان‌گیری و امنیت، خط قرمز ماست." },
    ...
    <div ref={ref} className="bg-paper">
      <section className="grid-lines grid-lines-fade noise relative overflow-hidden bg-ink-950 pb-20 pt-14 sm:pt-20">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
          <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <p className="reveal eyebrow text-teal-400">TSOFT · CAPITAL · SINCE 2006</p>
              ...
              <p className="reveal mt-6 text-lg leading-10 text-mist-300">
                تیسافت سال {fa(1385)} با یک ایده ساده شروع شد: نرم‌افزار حسابداریِ فروشگاه نباید از خودِ فروشگاه پیچیده‌تر باشد. اولین نسخه را برای یک سوپرمارکت نوشتیم — و هنوز همان مشتری، مشتری ماست.
              </p>
              <p className="reveal leading-9 text-mist-300" style={{ "--rv-delay": "150ms" } as React.CSSProperties}>
                از همان روز اول، <b className="text-white">توسعه و پشتیبانی هرگز برون‌سپاری نشد</b>؛ هر نسخه، دست‌سازِ خودمان است. امروز دو محصول تخصصی داریم — تیسافت برای فروشگاه و کپیتال برای بازرگانی چندارزی — اما قانون اول عوض نشده:
            ...
            <div className="reveal rv-scale mx-auto flex flex-col items-center">
              ...
              <span className="mt-4 font-latin text-[10px] tracking-[0.3em] text-mist-300" dir="ltr">1385 — 1405</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <StatBox v={20} label="سال توسعه بی‌وقفه" />
            <StatBox v={2} label="محصول تخصصی" />
            <StatBox v={35} label="استان و شهر — ایران و افغانستان" />
            <StatBox v={1385} plain label="سالِ شروع — و هنوز در مسیر" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <p className="reveal eyebrow">OUR VALUES</p>
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2">
            {VALUES.map((v, i) => (
              <div key={v.title} className="reveal card-lift flex gap-5 rounded-3xl border border-ink-100 bg-paper p-8" style={{ "--rv-delay": `${(i % 2) * 100}ms` } as React.CSSProperties}>
                <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-ink-950 text-teal-400 transition-transform duration-300 hover:-rotate-6 hover:scale-110"><Icon name={v.icon} className="h-7 w-7" /></span>
                <div>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          ...
          <div className="reveal card-lift mt-10 rounded-3xl border border-ink-100 bg-white p-8 sm:p-10">
            <div className="flex items-center gap-4">
              <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-ink-950 text-teal-400"><Icon name="cpu" className="h-8 w-8" /></span>
              <div>
            ...
            <p className="mt-6 leading-9 text-mist-500">
              وقتی زنگ می‌زنید، همان کسی تلفن را جواب می‌دهد که کد را نوشته — واسطه‌ای در کار نیست؛ برای همین مشکلات همان‌جا، پشت تلفن حل می‌شوند.
            </p>
      ...
      <section className="border-t border-ink-100 bg-white py-16">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-4 sm:flex-row sm:px-6">
          ...
          <button onClick={() => nav({ page: "contact" })} className="btn-shine shrink-0 rounded-xl bg-gold-500 px-8 py-4 font-bold text-ink-950 transition-transform hover:scale-[1.02]">صحبت کنیم</button>
        </div>
      </section>
    </div>
  );
