        ...
        <div className="space-y-2">
          {chats?.map((c) => (
            <button
              key={c.user.id}
              onClick={() => open(c.user)}
              className={`flex w-full items-center gap-3 rounded-2xl border p-3.5 text-right transition-all ${
                sel?.id === c.user.id ? "border-teal-500/60 bg-teal-100/40" : "border-ink-100 bg-paper hover:border-teal-500/40"
              }`}
            >
              ...
              {c.unread > 0 && (
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#E14B4B] text-[10px] font-bold text-white">{fa(c.unread)}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {!sel ? (
          ...
          <>
            <div className="flex items-center gap-3 border-b border-ink-100 bg-paper px-6 py-4">
              <span className="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-ink-950 font-display text-lg text-teal-400">
                {sel.firstName[0]}
              </span>
              <div>
              ...
              {msgs.map((m) => (
                <div key={m.id} className={`ticker-in flex ${m.from === "admin" ? "justify-start" : "justify-end"}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4.5 py-3 text-sm leading-7 ${
                    m.from === "admin" ? "rounded-bl-md bg-teal-600 text-white" : "rounded-br-md border border-ink-100 bg-white text-ink-800"
                  }`} style={{ paddingLeft: 18, paddingRight: 18 }}>
                    <p className="whitespace-pre-wrap break-words">{m.text}</p>
                    <p className={`mt-1 text-[10px] ${m.from === "admin" ? "text-white/70" : "text-mist-300"}`}>
                      {fmtDate(m.time)}{m.from === "user" && m.read ? " · خوانده شد" : ""}
                    </p>
                  </div>
                </div>
              ))}
            ...
            <div className="flex items-center gap-3 border-t border-ink-100 bg-white p-4">
              <input
                ...
                placeholder={`پاسخ به ${sel.firstName}…`}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ink-100 bg-paper px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {active.status !== "closed" && (
              <button onClick={close} disabled={busy} className="rounded-full border border-[#E14B4B]/40 px-4 py-1.5 text-xs font-bold text-[#E14B4B] transition-colors hover:bg-[#E14B4B] hover:text-white disabled:opacity-50">
                بستن تیکت
              </button>
            )}
          </div>
        ...
        {([["all", "همه"], ["open", "باز"], ["answered", "پاسخ داده"], ["closed", "بسته"]] as const).map(([id, label]) => (
          <button
            ...
            className={`rounded-xl border px-4 py-2.5 text-xs font-bold transition-all ${
              filter === id ? "border-transparent bg-ink-950 text-white" : "border-ink-100 bg-white text-mist-500 hover:border-teal-500/50 hover:text-teal-600"
            }`}
          >
            {label}
          </button>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              ...
              <h1 className="reveal mt-3">
                <span className="line-mask"><span className="font-display text-5xl leading-none text-white sm:text-6xl">پنل مدیریت</span></span>
              </h1>
              <p className="reveal mt-3 text-sm text-mist-300">
                خوش آمدید، {user.firstName} — گفتگوها، تیکت‌ها و کاربران اینجاست.
              </p>
            </div>
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`flex items-center gap-2.5 rounded-xl border px-5 py-3 text-sm font-bold transition-all ${
                  tab === t.id ? "border-transparent bg-ink-950 text-white shadow-lg" : "border-ink-100 bg-white text-mist-500 hover:border-gold-500/50 hover:text-gold-600"
                }`}
              >
