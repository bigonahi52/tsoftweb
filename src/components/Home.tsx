...
function ProductsShowcase({ nav }: { nav: NavFn }) {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) setActive(e.target.id.replace("panel-", ""));
        });
      },
  ...
  return (
    <section id="products" className="relative scroll-mt-24 bg-paper py-32 sm:py-40">
        ...
        <div className="grid gap-16 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
          <div className="lg:sticky lg:top-28 lg:self-start">
            ...
            <h2 className="reveal mt-4">
              <span className="line-mask"><span className="font-display text-5xl leading-tight text-ink-900 sm:text-6xl">پنج محصول،</span></span>
            ...
            <p className="reveal mt-6 max-w-md leading-9 text-mist-500">
              هر کسب‌وکار، نرم‌افزارِ خودش را می‌خواهد. اسکرول کنید تا محصولات را یکی‌یکی ببینید.
            </p>
              ...
              {products.map((p, i) => (
                <button
                  ...
                  className={`flex w-full items-center gap-4 rounded-2xl border px-5 py-4 text-right transition-all duration-300 ${
                    active === p.id
                      ? "border-transparent bg-ink-950 text-white shadow-[0_18px_40px_-20px_rgba(10,27,33,0.5)]"
                      : "border-ink-100 bg-white text-ink-800 hover:border-teal-500/50"
                  }`}
                >
          ...
          <div className="space-y-14">
            {products.map((p) => (
              <div
                key={p.id}
                id={`panel-${p.id}`}
                ref={(el) => { panels.current[p.id] = el; }}
                className="reveal card-lift relative overflow-hidden rounded-3xl border border-ink-100 bg-white p-8 sm:p-10"
              >
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-latin text-[11px] tracking-[0.3em] text-mist-300">{p.latin}</p>
                  </div>
...
function WhyTisaft() {
  const reasons = [
    ...
    { icon: "update", title: "همیشه به‌روز", desc: "آپدیت‌ها رایگان‌اند و قوانین مالیاتی و حسابداری ایران در آن‌ها لحاظ می‌شود." },
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {reasons.map((r, i) => (
              <div key={r.title} className={`reveal card-lift rounded-3xl border border-ink-100 bg-white p-8 ${i % 3 === 0 ? "sm:translate-y-6" : ""}`} style={{ "--rv-delay": `${(i % 2) * 110}ms` } as React.CSSProperties}>
                <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-ink-950 text-teal-400 transition-transform duration-300 hover:scale-110 hover:-rotate-6">
...
/* ───────── نوار پیسافت ───────── */
function PisoftBand({ nav }: { nav: NavFn }) {
  const rows: [string, string, string, string][] = [
    ["یکپارچگی ماژول‌های مالی", "کامل", "کامل", "—"],
    ["حسابداری تولید و بهای تمام‌شده", "کامل", "کامل", "—"],
    ["هزینه لایسنس و استقرار", "منصفانه", "سنگین", "ارزان"],
    ["سرعت راه‌اندازی", "چند هفته", "چند ماه", "فوری"],
    ["پشتیبانی مستقیم از توسعه‌دهنده", "بله", "با واسطه", "محدود"],
    ["یک سال پشتیبانی رایگان", "دارد", "ندارد", "ندارد"],
  ];
  return (
    <section className="relative overflow-hidden bg-teal-600 py-28 sm:py-36">
      <div className="noise pointer-events-none absolute inset-0" />
      <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-4 sm:px-6 lg:grid-cols-2 lg:gap-20">
        <div>
          <p className="reveal font-latin text-xs tracking-[0.3em] text-teal-100">PSOFT ENTERPRISE</p>
          <h2 className="mt-3">
            <span className="line-mask"><span className="font-display text-5xl leading-tight text-[#ffffff] sm:text-6xl">حسابداری سازمانی،</span></span>
            <span className="line-mask" style={{ "--rv-delay": "120ms" } as React.CSSProperties}>
              <span className="font-display text-5xl leading-tight text-ink-950 sm:text-6xl">به قیمتِ باورنکردنی</span>
            </span>
          </h2>
          <p className="reveal mt-6 max-w-lg leading-9 text-teal-100">
            پیسافت برای شرکت‌ها و کارخانجاتی ساخته شده که به یک حسابداری یکپارچه و کامل نیاز دارند — همان عمقِ
            راهکارهای سازمانی بزرگ، اما سبک‌تر، چابک‌تر و با هزینه‌ای منطقی.
            بخرید، یک سال کامل پشتیبانی رایگان دارید؛ بعد از آن خودتان تصمیم می‌گیرید.
          </p>
          <div className="reveal mt-8 flex flex-wrap gap-4">
            <button onClick={() => nav({ page: "product", id: "pisoft" })} className="btn-shine group flex items-center gap-2.5 rounded-xl bg-ink-950 px-7 py-3.5 font-bold text-white transition-transform hover:scale-[1.02]">
              آشنایی با پیسافت
              <Icon name="arrow" className="h-4 w-4 text-gold-400 transition-transform group-hover:-translate-x-1" />
            </button>
            <button onClick={() => nav({ page: "contact" })} className="rounded-xl border border-[#ffffff]/40 px-7 py-3.5 font-semibold text-[#ffffff] transition-colors hover:border-[#ffffff] hover:bg-[#ffffff]/10">
              دمو برای تیم مالی شما
            </button>
          </div>
        </div>

        <div className="reveal rv-left overflow-hidden rounded-3xl bg-ink-950/25 backdrop-blur-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[520px] text-sm">
              <thead>
                <tr className="border-b border-white/15 text-right">
                  <th className="px-5 py-4 font-medium text-teal-100">مقایسه</th>
                  <th className="px-3 py-4 font-display text-lg text-gold-400">پیسافت</th>
                  <th className="px-3 py-4 font-normal text-teal-100/80">راهکارهای سازمانی گران</th>
                  <th className="px-3 py-4 font-normal text-teal-100/80">نرم‌افزارهای فروشگاهی ساده</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r[0]} className="border-b border-white/10 last:border-0">
                    <td className="px-5 py-3.5 text-teal-100">{r[0]}</td>
                    {r.slice(1).map((c, i) => (
                      <td key={i} className={`px-3 py-3.5 text-center ${i === 0 ? "font-bold text-gold-400" : "text-teal-100/75"}`}>{c}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
...
/* ───────── نظرات مشتریان ───────── */
function Voices() {
  const quotes = products.filter((p) => p.quote).map((p) => ({ ...p.quote!, product: p.name, accent: p.accent, name: p.quote!.author }));
  return (
    <section className="bg-paper py-28 sm:py-36">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {quotes.map((q, i) => (
            <figure key={q.product} className="reveal card-lift relative rounded-3xl border border-ink-100 bg-white p-7 pt-10" style={{ "--rv-delay": `${(i % 5) * 90}ms` } as React.CSSProperties}>
              <span className="absolute -top-5 right-7 flex h-10 w-10 items-center justify-center rounded-full font-display text-4xl text-[#ffffff]" style={{ background: q.accent }} aria-hidden>”</span>
              ...
              <figcaption className="mt-5 border-t border-ink-50 pt-4">
                <p className="font-display text-lg text-ink-900">{q.name}</p>
  ...
  return (
    <section className="relative overflow-hidden border-t border-ink-100 bg-white py-28 sm:py-36">
      <div className="mx-auto max-w-5xl px-4 text-center sm:px-6">
        <span className="seal-pulse reveal mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-gold-100 text-gold-600" aria-hidden>
        ...
        <p className="reveal mx-auto mt-5 max-w-2xl leading-9 text-mist-500">
          تک‌تک محصولات تیسافت با علاقه‌ی واقعی به کارِ شما طراحی شده‌اند — و رضایت شما، بزرگ‌ترین تبلیغ ماست.
        </p>
        <div className="reveal mt-12 grid gap-5 sm:grid-cols-2">
          <div className="card-lift rounded-3xl border border-ink-100 bg-paper p-8 text-right">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-teal-500/15 text-teal-600">
    ...
    <section className="grid-lines grid-lines-fade noise relative overflow-hidden bg-ink-950 py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6">
        ...
        <p className="reveal mx-auto mt-6 max-w-xl leading-9 text-mist-300">
          نسخه آزمایشی همه محصولات رایگان است؛ نصب کنید، با داده‌های خودتان تست کنید و بعد تصمیم بگیرید.
        ...
        <div className="reveal mt-9 flex flex-wrap justify-center gap-4">
          ...
          <button onClick={() => nav({ page: "contact" })} className="rounded-xl border border-ink-600 px-8 py-4 text-lg font-semibold text-ink-100 transition-colors hover:border-gold-500 hover:text-gold-400">
            مشاوره با کارشناس
          </button>
        </div>
      </div>
